...
export default function ForecastTab() {
  const { forecastScenarios, summary, baseDate, maxFileDate, config } = usePrepurchase();

  const today = new Date();
  const showWarning = maxFileDate && maxFileDate < today;

  // Get a representative "medium" scenario for projection chart
  ...
  // Multi-scenario comparison (pick Historico completo group)
  ...
  // Build multi-line comparison data
  const comparisonData = useMemo(() => {
    ...
    for (let d = 0; d <= maxDias; d += step) {
      ...
      for (const sc of scenarioGroups) {
        if (!sc) continue;
        const saldo = Math.max(0, summary.saldoComercialDisponible - sc.consumoDiario * d);
        pt[sc.escenario.replace(' (Histórico completo)', '')] = saldo;
      }
      points.push(pt);
    }
    return points;
  ...
  // Days remaining bar data
  const daysBarData = useMemo(() => {
    const topScenarios = [
      'Conservador (Histórico completo)',
      'Medio (Histórico completo)',
      'Acelerado (Histórico completo)',
      'Conservador (Últimos 30 días)',
      'Medio (Últimos 30 días)',
      'Manual',
    ];
    return forecastScenarios
      .filter((s) => topScenarios.includes(s.escenario))
      .map((s) => ({
  ...
  if (!summary) {
    return <div className="p-6 text-center text-gray-500">No hay datos para calcular el pronóstico.</div>;
  }
    ...
    <div className="p-6 space-y-6">
      ...
      {showWarning && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
          ...
          {maxFileDate!.toLocaleDateString('es-CO', { dateStyle: 'long' })}. La proyección se calcula
          desde esa fecha. Puede cambiar la fecha base en{' '}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">Indicador de Agotamiento</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-xl">
            <p className="text-xs text-blue-600 font-medium">Porcentaje Consumido</p>
            <p className="text-3xl font-bold text-blue-800 mt-2">
              {(summary.porcentajeConsumido * 100).toFixed(1)}%
            </p>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-xl">
            <p className="text-xs text-green-600 font-medium">Saldo Disponible</p>
            <p className="text-lg font-bold text-green-800 mt-2">{formatCOP(summary.saldoComercialDisponible)}</p>
          </div>
          {mediumScenario && (
            <>
              <div className="text-center p-4 bg-purple-50 rounded-xl">
                <p className="text-xs text-purple-600 font-medium">Fecha Estimada</p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <button
            onClick={() => exportPronostico(forecastScenarios)}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="px-3 py-2.5 text-left font-semibold text-gray-600 whitespace-nowrap">Escenario</th>
                <th className="px-3 py-2.5 text-right font-semibold text-gray-600 whitespace-nowrap">Consumo Diario</th>
                <th className="px-3 py-2.5 text-right font-semibold text-gray-600 whitespace-nowrap">Consumo Semanal</th>
                <th className="px-3 py-2.5 text-right font-semibold text-gray-600 whitespace-nowrap">Consumo Mensual</th>
                <th className="px-3 py-2.5 text-right font-semibold text-gray-600 whitespace-nowrap">Días Rest.</th>
                <th className="px-3 py-2.5 text-right font-semibold text-gray-600 whitespace-nowrap">Semanas Rest.</th>
                <th className="px-3 py-2.5 text-center font-semibold text-gray-600 whitespace-nowrap">Fecha Estimada</th>
                <th className="px-3 py-2.5 text-center font-semibold text-gray-600">Riesgo</th>
                <th className="px-3 py-2.5 text-left font-semibold text-gray-600">Observación</th>
              </tr>
            </thead>
              ...
              {forecastScenarios.map((s, i) => (
                <tr key={i} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 py-2 text-center text-gray-700 whitespace-nowrap">
                    {s.fechaEstimada
                      ? s.fechaEstimada.toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })
                      : 'N/D'}
                  </td>
                  <td className="px-3 py-2 text-center">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${riskBg(s.nivelRiesgo)}`}>
                      {s.nivelRiesgo.toUpperCase()}
                    </span>
                  </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Línea de Agotamiento Proyectado (Escenario Medio)</h3>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={projectionData}>
              ...
              <Line type="monotone" dataKey="saldo" name="Saldo Proyectado" stroke="#3b82f6" dot={false} strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Scenario comparison */}
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Comparación de Escenarios (Histórico completo)</h3>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={comparisonData}>
              ...
              <Line type="monotone" dataKey="Medio" stroke="#3b82f6" dot={false} strokeWidth={2} />
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={daysBarData} layout="vertical">
              ...
              <Tooltip
                formatter={(v) => [Number(v) + ' días', 'Días restantes']}
              />
              <Bar dataKey="dias" name="Días restantes" radius={[0, 2, 2, 0]}>
                {daysBarData.map((entry, i) => (
                  <Cell key={i} fill={entry.riesgo === 'alto' ? '#ef4444' : entry.riesgo === 'medio' ? '#eab308' : '#22c55e'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">Fecha base usada</span>
              <span className="font-semibold text-gray-900">
                {baseDate.toLocaleDateString('es-CO', { dateStyle: 'long' })}
              </span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">Saldo disponible</span>
              <span className="font-semibold text-gray-900">{formatCOP(summary.saldoComercialDisponible)}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">Consumo promedio diario</span>
              <span className="font-semibold text-gray-900">{formatCOP(summary.promedioDiario)}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">Consumo promedio semanal</span>
              <span className="font-semibold text-gray-900">{formatCOP(summary.promedioSemanal)}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">Consumo promedio mensual</span>
              <span className="font-semibold text-gray-900">{formatCOP(summary.promedioMensual)}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-gray-500">Escenarios calculados</span>
              <span className="font-semibold text-gray-900">{forecastScenarios.length}</span>
            </div>
          </div>
        </div>
