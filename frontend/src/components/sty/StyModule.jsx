// Módulo STY — gemelo de OtbModule: misma lógica de parseo/comparación/UI, colección Mongo
// separada (sty_analyses). Reusa a propósito las libs `lib/otb/*` y los componentes
// presentacionales Otb* (Charts/Consultant/TechSheet/OccVsBaseChart) y las clases CSS
// `otb-*` de otb.css — son genéricos, no tienen nada específico de OTB en su render.
...
import OtbCharts from '../otb/OtbCharts.jsx';
import OtbConsultant from '../otb/OtbConsultant.jsx';
import OtbTechSheet from '../otb/OtbTechSheet.jsx';
import OtbOccVsBaseChart from '../otb/OtbOccVsBaseChart.jsx';
...
export default function StyModule({ session }) {
  ...
  const loadHistory = () => {
    ...
    apiFetch(`/sty/analyses?hotelId=${encodeURIComponent(hotel)}`).then((r) => setHistory(r.analyses ?? [])).catch(() => setHistory([]));
  };
  ...
  const viewSaved = async (id) => {
    setError('');
    try {
      const r = await apiFetch(`/sty/analyses/${id}`);
  ...
  const deleteHistory = async (id) => {
    ...
    setError('');
    try {
      await apiFetch(`/sty/analyses/${id}`, { method: 'DELETE' });
    ...
    <div>
      ...
      {tab === 'comparison' && metrics && comparison && (
        <div>
          ...
          <OtbTechSheet metrics={metrics} weeklyMetrics={weeklyMetrics} moduleLabel="STY" />
